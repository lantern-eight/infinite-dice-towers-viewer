...
#### 1. Install Pillow (one time only)

Pillow makes the thumbnail images smaller so the catalog file isn't huge. Open **Terminal** (search for "Terminal" in Spotlight, or find it in Applications > Utilities) and paste this command, then press Enter:
...
#### 2. Install Pillow (one time only)

Pillow makes the thumbnail images smaller so the catalog file isn't huge. Open **Command Prompt** (search for "cmd" in the Start menu) and paste this command, then press Enter:
